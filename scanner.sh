...
echo "[+] AMASS SCANNING [+]"
		amass enum -brute -w ~/Automated-Scanner/tools/subdomains.txt -rf ~/tools/resolvers.txt -d $domain -o ~/assets/$domain/amass.txt
...
echo "[+] MASSCAN PORT SCANNING [+]"
	...
	mass=$(cat ~/assets/$domain/masscan.txt | grep "Host:" | awk {'print $5'} | awk -F '/' {'print $1'} | sort -u | wc -l)
	$message "Masscan%20discovered%20$mass%20open%20port(s)%20for%20$domain"
	echo "[+] Done masscan for scanning port(s)"

big_ports=$(cat ~/assets/$domain/masscan.txt | grep "Host:" | awk {'print $5'} | awk -F '/' {'print $1'} | sort -u | paste -s -d ',')
cat ~/assets/$domain/masscan.txt | grep "Host:" | awk {'print $2":"$5'} | awk -F '/' {'print $1'} | sed 's/:80$//g' | sed 's/:443$//g' | sort -u > ~/assets/$domain/open-ports.txt  
...
echo "[+] COLLECTING ENDPOINTS [+]"
	for urlz in $(cat ~/assets/$domain/httprobe.txt); do 
		...
		link=$(python3 ~/tools/LinkFinder/linkfinder.py -i $urlz -d -o cli | grep -E "*.js$" | grep "$domain" | grep "Running against:" |awk {'print $3'})
		echo "Running against: $urlz"
		if [[ ! -z $link ]]; then
			for linx in $link; do
				python3 ~/tools/LinkFinder/linkfinder.py -i $linx -o cli > ~/assets/$domain/endpoints/$filename-result.txt
			done
		else
			python3 ~/tools/LinkFinder/linkfinder.py -i $urlz -d -o cli > ~/assets/$domain/endpoints/$filename-result.txt
		fi
	done
...
echo "[+] COLLECTING SECRETS FROM GITHUB [+]"
	for url in $(cat ~/assets/$domain/alive.txt ); do 
		u=$(echo $url | sed 's/\./\\./g');
		echo "Running against: $url"
		python3 ~/tools/github-secrets.py -s $u -t $(cat ~/Automated-Scanner/tools/.tokens) > ~/assets/$domain/github-secrets/$url.txt
	done
...
echo "[+] ALIENVAULT, WAYBACKURLS and COMMON CRAWL Scanning for Archived Endpoints [+]"
	for u in $(cat ~/assets/$domain/final.txt | awk '{print $1}');do echo $u | gau  >> ~/assets/$domain/gau/tmp-$u.txt; done
	cat ~/assets/$domain/gau/* | sort -u  >> ~/assets/$domain/gau/gau.txt 
...
echo "[+] DALFOX for injecting blindxss"
	 cat ~/assets/$domain/final.txt | awk '{print $1}' | gau | grep "=" | dalfox pipe -b $xss_hunter -o ~/assets/$domain/dalfox.txt
	$message "DALFOX%20Done%20for%20$domain"
	echo "[+] Done dalfox for injecting blind xss"

echo "[+] KXSS for potential vulnerable xss"
	 cat ~/assets/$domain/final.txt | awk '{print $1}' | gau | grep "=" | kxss | grep "is reflected and allows" | awk {'print $9'} | sort -u >> ~/assets/$domain/kxss/kxss.txt
